...
export class PoiRenderer {
    ...
    static computeIconScreenBox(
        ...
        const iconXOffset = getNumberPropertyValueSafe(technique.iconXOffset, 0, zoomLevel);
        const iconYOffset = getNumberPropertyValueSafe(technique.iconYOffset, 0, zoomLevel);
    ...
    private setupPoiInfo(
        ...
        const screenWidth = getNumberPropertyValueSafe(technique.screenWidth, undefined, zoomLevel);
        if (screenWidth !== undefined) {
            iconScaleV = iconScaleH = screenWidth / iconWidth;
        }

        const screenHeight = getNumberPropertyValueSafe(
            technique.screenHeight,
            undefined,
            zoomLevel
        );
